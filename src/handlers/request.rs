...
fn sync_request_with_app_state(app: &mut App) -> Result<()> {
    let current_tab = app.tabs.get_mut(app.selected_tab)
...
pub fn set_request_content_type(app: &mut App, content_type: &str) -> Result<()> {
    ...
    app.headers_input.retain(|(key, _)| key.to_lowercase() != "content-type");
    
    // Add new content-type header
    app.headers_input.push(("Content-Type".to_string(), content_type.to_string()));
...
mod tests {
    ...
    fn test_get_request_content_type() {
        ...
        app.headers_input.push(("Content-Type".to_string(), "application/json".to_string()));
        assert_eq!(get_request_content_type(&app), Some("application/json".to_string()));
        
        // Case insensitive
        app.headers_input.clear();
        app.headers_input.push(("content-type".to_string(), "text/plain".to_string()));
        assert_eq!(get_request_content_type(&app), Some("text/plain".to_string()));
    }
    ...
    fn test_format_request_info() {
        let mut app = create_test_app();
        app.headers_input.push(("Accept".to_string(), "application/json".to_string()));
        app.params_input.push(("limit".to_string(), "10".to_string()));
